...
my_input_names <- standard_df_all %>% dplyr::select(-obs) %>% names()
my_top_ranked_inputs <- c("xb_07","xw_01")

viz_input_grid <- make_test_input_grid(my_input_names, my_top_ranked_inputs, standard_df_all)
```

## Model Import

```{r}
rstanarm_cat_mod3 <- readr::read_rds('models/rstanarm_cat_mod3.rds')
rstanarm_cat_mod9 <- readr::read_rds('models/rstanarm_cat_mod9.rds')
```
...
```{r}
post_pred_mod3 <- posterior_predict(rstanarm_cat_mod3, newdata = viz_input_grid)
```
...
post_pred_mod3 %>% 
  ...
  ungroup() %>% 
  left_join(posterior_linpred(rstanarm_cat_mod3, newdata = viz_input_grid) %>% 
              as.data.frame() %>% tibble::as_tibble() %>% 
              tibble::rowid_to_column("post_id") %>% 
              tidyr::gather(key = "pred_id", value = "value", -post_id) %>% 
              mutate_at(c("pred_id"), as.numeric) %>% 
...
```{r}
posterior_predict(rstanarm_cat_mod3, newdata = viz_input_grid) %>% 
  as.data.frame() %>% tibble::as_tibble() %>% 
  tibble::rowid_to_column("post_id") %>% 
  tidyr::gather(key = "pred_id", value = "value", -post_id) %>% 
  mutate_at(c("pred_id"), as.numeric) %>% 
  ...
  left_join(viz_input_grid %>% tibble::rowid_to_column("pred_id"),
            by = "pred_id") %>% 
  ggplot(mapping = aes(x = xb_07, y = prob_grt_0)) +
...
```{r}
post_pred_mod9 <- posterior_predict(rstanarm_cat_mod9, newdata = viz_input_grid)
```
...
post_pred_mod9 %>% 
  ...
  summarise(y_avg = mean(value),
            ...
            y_upr = quantile(value, 0.95)) %>% 
  ungroup() %>% 
  left_join(posterior_linpred(rstanarm_cat_mod9, newdata = viz_input_grid) %>% 
              as.data.frame() %>% tibble::as_tibble() %>% 
              tibble::rowid_to_column("post_id") %>% 
              tidyr::gather(key = "pred_id", value = "value", -post_id) %>% 
              mutate_at(c("pred_id"), as.numeric) %>% 
...
```{r}
posterior_predict(rstanarm_cat_mod9, newdata = viz_input_grid) %>% 
  as.data.frame() %>% tibble::as_tibble() %>% 
  tibble::rowid_to_column("post_id") %>% 
  tidyr::gather(key = "pred_id", value = "value", -post_id) %>% 
  mutate_at(c("pred_id"), as.numeric) %>% 
