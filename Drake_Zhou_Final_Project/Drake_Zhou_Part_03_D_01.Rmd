...
set.seed(1024)
fit_acc_cat_lm_pair <- train(obs ~ region + customer + (.)^2,
                  data = standard_df_all,
                  method = "glm",
                  metric = my_metric,
                  preProcess = c("center", "scale"),
...
```{r}
set.seed(1024)
fit_acc_cat_lm_mod1 <- train(obs ~ region + customer,
                  data = standard_df_all,
                  method = "glm",
                  metric = my_metric,
                  preProcess = c("center", "scale"),
...
```{r, message=FALSE, warning=FALSE}
set.seed(1024)
fit_acc_cat_enet_pair <- train(obs ~ region + customer + (.)^2,
                               data = standard_df_all,
                               method = "glmnet",
                               metric = my_metric,
                               preProcess = c("center", "scale"),
...
set.seed(1024)
fit_acc_cat_enet_mod7 <- train(formula_mod7,
                       data = standard_df_all,
                       method = "glmnet",
                       metric = my_metric,
                       preProcess = c("center", "scale"),
...
set.seed(1024)
fit_acc_cat_enet_mod9 <- train(formula_mod9,
                       data = standard_df_all,
                       method = "glmnet",
                       metric = my_metric,
                       preProcess = c("center", "scale"),
...
fit_acc_cat_nnet_warmup <- train(obs ~ .,
                    data = standard_df_all,
                    method = "nnet",
                    metric = my_metric,
                    preProcess = c("center", "scale"),
...
fit_acc_cat_nnet_warmup
```

Visualize the result and find tuning parameters

```{r}
plot(fit_acc_cat_nnet_warmup, xTrans = log)
```
...
fit_acc_cat_nnet_tune <- train(obs ~ .,
                    data = standard_df_all,
                    method = "nnet",
                    metric = my_metric,
...
plot(fit_acc_cat_nnet_tune, xTrans = log)
```
...
```{r}
set.seed(12341)
fit_acc_cat_gbm_warmup <- train(obs ~ .,
                      data = standard_df_all,
                      method = "gbm",
                      metric = my_metric,
                      trControl = my_ctrl,
...
gbm_grid <- expand.grid(n.trees = c(100, 150, 300, 500, 750, 1000),
                        ...
                        interaction.depth = fit_acc_cat_gbm_warmup$bestTune$interaction.depth,
                        n.minobsinnode = fit_acc_cat_gbm_warmup$bestTune$n.minobsinnode)

set.seed(12341)
fit_acc_cat_gbm_tune <- train(obs ~ .,
                      data = standard_df_all,
                      method = "gbm",
                      metric = my_metric,
                      tuneGrid = gbm_grid,
...
```{r}
fit_acc_cat_gbm_tune
plot(fit_acc_cat_gbm_tune)
```
...
```{r}
set.seed(1024)
fit_acc_cat_svm_tune <- train(obs ~ .,
                 data = standard_df_all,
                 method = "svmRadial",
                 metric = my_metric,
                 preProcess = c("center", "scale"),
...
```{r}
my_performance <- resamples(list(LM_1 = fit_acc_cat_lm_add,
...
```{r}
dotplot(my_performance, metric = "Accuracy")
```
...
```{r}
fit_acc_cat_lm_add %>% readr::write_rds('models/fit_acc_cat_lm_add.rds')
fit_acc_cat_lm_pair %>% readr::write_rds('models/fit_acc_cat_lm_pair.rds')
fit_acc_cat_lm_mod1 %>% readr::write_rds('models/fit_acc_cat_lm_mod1.rds')
fit_acc_cat_lm_mod2 %>% readr::write_rds('models/fit_acc_cat_lm_mod2.rds')
fit_acc_cat_enet_pair %>% readr::write_rds('models/fit_acc_cat_enet_pair.rds')
fit_acc_cat_enet_mod7 %>% readr::write_rds('models/fit_acc_cat_enet_mod7.rds')
fit_acc_cat_enet_mod9 %>% readr::write_rds('models/fit_acc_cat_enet_mod9.rds')
fit_acc_cat_gbm_warmup %>% readr::write_rds('models/fit_acc_cat_gbm_warmup.rds')
fit_acc_cat_gbm_tune %>% readr::write_rds('models/fit_acc_cat_gbm_tune.rds')
fit_acc_cat_nnet_warmup %>% readr::write_rds('models/fit_acc_cat_nnet_warmup.rds')
fit_acc_cat_nnet_tune %>% readr::write_rds('models/fit_acc_cat_nnet_tune.rds')
fit_acc_cat_rf_tune %>% readr::write_rds('models/fit_acc_cat_rf_tune.rds')
fit_acc_cat_svm_tune %>% readr::write_rds('models/fit_acc_cat_svm_tune.rds')
fit_acc_cat_cls_pls %>% readr::write_rds('models/fit_acc_cat_cls_pls.rds')
```